...
function Comment(
  props: comment
    & { prevId: string, nextId: string, parentId: string}
): React.ReactElement {

  const [commentOpen, setCommentOpen] = useState<boolean>(true);
  const numOfComments = getNumOfComments(props.comments)
  const indentSpace = props.level < 3
    ? props.level
    : 2 + ((props.level - 2) * 0.1) // indent less when too deep level
          ...
          <table border={0} cellPadding="0" cellSpacing="0">
            <tbody>
              {(props.comments.map((comment, index) => {
                ...
                const nextId = props.comments.length === index + 1
                  ? ''
                  : props.comments[index + 1].id
                return (
                  <Comment
  ...
  return (
    <>
      ...
      {data.comments?.length > 0 && (data.comments.map((comment, index) => {
        ...
        const nextId = data.comments.length === index + 1
          ? ''
          : data.comments[index + 1].id
        return (
          <Comment
